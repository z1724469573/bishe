...
@Tag(name = "评论")
@RestController
@RequestMapping("/reviewEntity")
public class ReviewController {

    @Autowired
    private ReviewMapper commonMapper;
    @Autowired
    private ReviewService commonService;
    ...
    @PostMapping("/exist")
    public Result existReview(@RequestBody @Validated IdParam param) {
        QueryWrapper<ReviewEntity> wrapper = new QueryWrapper<>();
    ...
    @PostMapping("/insert")
    public Result insertReview(@RequestBody @Validated ReviewEntity param) {
        QueryWrapper<ReviewEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getReviewId());
    ...
    @PostMapping("/delete")
    public Result deleteReview(@RequestBody @Validated IdParam param) {
        QueryWrapper<ReviewEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
    ...
    @PostMapping("/update")
    public Result updateReview(@RequestBody @Validated ReviewEntity param) {
        QueryWrapper<ReviewEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getReviewId());
    ...
    @PostMapping("/select")
    public Result selectReview(@RequestBody @Validated IdParam param) {
        QueryWrapper<ReviewEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
        if (commonService.count(wrapper) == 0) return Result.failure("ID不存在", param.getId());
    ...
    public Result listPageReview(@RequestParam Integer page, @RequestParam Integer pageSize) {
        //分页参数
        Page<ReviewEntity> rowPage = new Page(page, pageSize);
        //queryWrapper组装查询where条件
        LambdaQueryWrapper<ReviewEntity> queryWrapper = new LambdaQueryWrapper<>();
    ...
    public void exportExcelReview(HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("轮播图", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
        List<ReviewEntity> list = commonService.list();
        response.setHeader("Content-disposition", "attachment;filename*=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), ReviewEntity.class).sheet("轮播图").doWrite(list);
    }

    @Operation(summary = "导入数据")
    @PostMapping("/importExcel")
    public Result importExcelReview(MultipartFile file) {
        try {
            //获取文件的输入流
            InputStream inputStream = file.getInputStream();
            List<ReviewEntity> list = EasyExcel.read(inputStream) //调用read方法
                    //注册自定义监听器，字段校验可以在监听器内实现
                    //.registerReadListener(new UserListener())
                    .head(ReviewEntity.class) //对应导入的实体类
