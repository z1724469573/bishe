...
import com.example.springboot.entity.ElderEntity;
import com.example.springboot.mapper.ElderMapper;
import com.example.springboot.service.ElderService;
...
public class ElderController {
    ...
    @Autowired
    private ElderMapper commonMapper;
    @Autowired
    private ElderService commonService;
    private final String id = "Elder_id";

    @Operation(summary = "列表")
    @PostMapping("/list")
    public Result listElder() {
        return Result.success("数据列表", commonService.list());
    }

    @Operation(summary = "存在")
    @PostMapping("/exist")
    public Result existElder(@RequestBody @Validated IdParam param) {
    ...
    @PostMapping("/insert")
    public Result insertElder(@RequestBody @Validated ElderEntity param) {
        QueryWrapper<ElderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getElderId());
    ...
    @PostMapping("/delete")
    public Result deleteElder(@RequestBody @Validated IdParam param) {
        QueryWrapper<ElderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
    ...
    @PostMapping("/update")
    public Result updateElder(@RequestBody @Validated ElderEntity param) {
        QueryWrapper<ElderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getElderId());
    ...
    @PostMapping("/select")
    public Result selectElder(@RequestBody @Validated IdParam param) {
        QueryWrapper<ElderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
        if (commonService.count(wrapper) == 0) return Result.failure("ID不存在", param.getId());
    ...
    public Result listPageElder(@RequestParam Integer page, @RequestParam Integer pageSize) {
        //分页参数
        Page<ElderEntity> rowPage = new Page(page, pageSize);
        //queryWrapper组装查询where条件
        LambdaQueryWrapper<ElderEntity> queryWrapper = new LambdaQueryWrapper<>();
    ...
    public void exportExcelElder(HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("轮播图", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
        List<ElderEntity> list = commonService.list();
        response.setHeader("Content-disposition", "attachment;filename*=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), ElderEntity.class).sheet("轮播图").doWrite(list);
    }

    @Operation(summary = "导入数据")
    @PostMapping("/importExcel")
    public Result importExcelElder(MultipartFile file) {
        try {
            //获取文件的输入流
            InputStream inputStream = file.getInputStream();
            List<ElderEntity> list = EasyExcel.read(inputStream) //调用read方法
                    //注册自定义监听器，字段校验可以在监听器内实现
                    //.registerReadListener(new UserListener())
                    .head(ElderEntity.class) //对应导入的实体类
                    .sheet(0) //导入数据的sheet页编号，0代表第一个sheet页，如果不填，则会导入所有sheet页的数据
                    .headRowNumber(1) //列表头行数，1代表列表头有1行，第二行开始为数据行
                    .doReadSync();//开始读Excel，返回一个List<T>集合，继续后续入库操作
            //模拟导入数据库操作
            for (ElderEntity entity : list) {
