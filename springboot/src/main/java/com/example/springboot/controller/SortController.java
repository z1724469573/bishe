...
import com.example.springboot.entity.SortEntity;
import com.example.springboot.entity.SortEntity;
import com.example.springboot.mapper.SortMapper;
...
public class SortController {
    ...
    @Autowired
    private SortMapper commonMapper;
    @Autowired
    private SortService commonService;
    private final String id = "Sort_id";
    ...
    @PostMapping("/exist")
    public Result existSort(@RequestBody @Validated IdParam param) {
    ...
    @PostMapping("/insert")
    public Result insertSort(@RequestBody @Validated SortEntity param) {
        QueryWrapper<SortEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getSortId());
    ...
    @PostMapping("/delete")
    public Result deleteSort(@RequestBody @Validated IdParam param) {
        QueryWrapper<SortEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
    ...
    @PostMapping("/update")
    public Result updateSort(@RequestBody @Validated SortEntity param) {
        QueryWrapper<SortEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getSortId());
    ...
    @PostMapping("/select")
    public Result selectSort(@RequestBody @Validated IdParam param) {
        QueryWrapper<SortEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
        if (commonService.count(wrapper) == 0) return Result.failure("ID不存在", param.getId());
    ...
    public Result listPageSort(@RequestParam Integer page, @RequestParam Integer pageSize) {
        //分页参数
        Page<SortEntity> rowPage = new Page(page, pageSize);
        //queryWrapper组装查询where条件
        LambdaQueryWrapper<SortEntity> queryWrapper = new LambdaQueryWrapper<>();
    ...
    public void exportExcelSort(HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("轮播图", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
        List<SortEntity> list = commonService.list();
        response.setHeader("Content-disposition", "attachment;filename*=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), SortEntity.class).sheet("轮播图").doWrite(list);
    }

    @Operation(summary = "导入数据")
    @PostMapping("/importExcel")
    public Result importExcelSort(MultipartFile file) {
        try {
            //获取文件的输入流
            InputStream inputStream = file.getInputStream();
            List<SortEntity> list = EasyExcel.read(inputStream) //调用read方法
                    //注册自定义监听器，字段校验可以在监听器内实现
                    //.registerReadListener(new UserListener())
                    .head(SortEntity.class) //对应导入的实体类
