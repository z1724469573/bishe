...
public class OrderController {
    ...
    @Autowired
    private OrderMapper commonMapper;
    @Autowired
    ...
    @PostMapping("/exist")
    public Result existOrder(@RequestBody @Validated IdParam param) {
    ...
    @PostMapping("/insert")
    public Result insertOrder(@RequestBody @Validated OrderEntity param) {
        QueryWrapper<OrderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getOrderId());
    ...
    @PostMapping("/delete")
    public Result deleteOrder(@RequestBody @Validated IdParam param) {
        QueryWrapper<OrderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
    ...
    @PostMapping("/update")
    public Result updateOrder(@RequestBody @Validated OrderEntity param) {
        QueryWrapper<OrderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getOrderId());
    ...
    @PostMapping("/select")
    public Result selectOrder(@RequestBody @Validated IdParam param) {
        QueryWrapper<OrderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
        if (commonService.count(wrapper) == 0) return Result.failure("ID不存在", param.getId());
    ...
    public Result selectOrderByAcc(@PathVariable @Validated String param) {
        QueryWrapper<OrderEntity> wrapper = new QueryWrapper<>();
        wrapper.eq("order_acc", param);
    ...
    public Result listPageOrder(@RequestParam Integer page, @RequestParam Integer pageSize) {
        //分页参数
        Page<OrderEntity> rowPage = new Page(page, pageSize);
    ...
    public void exportExcelOrder(HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("轮播图", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
        List<OrderEntity> list = commonService.list();
        response.setHeader("Content-disposition", "attachment;filename*=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), OrderEntity.class).sheet("轮播图").doWrite(list);
    }

    @Operation(summary = "导入数据")
    @PostMapping("/importExcel")
    public Result importExcelOrder(MultipartFile file) {
        try {
            //获取文件的输入流
            InputStream inputStream = file.getInputStream();
            List<OrderEntity> list = EasyExcel.read(inputStream) //调用read方法
                    //注册自定义监听器，字段校验可以在监听器内实现
                    //.registerReadListener(new UserListener())
                    .head(OrderEntity.class) //对应导入的实体类
