...
import com.example.springboot.mapper.SwiperMapper;
...
@Tag(name = "轮播图")
@RestController
@RequestMapping("/swiperEntity")
public class SwiperController {

    @Autowired
    private SwiperMapper commonMapper;
    @Autowired
    private SwiperService commonService;
    private final String id = "Swiper_id";
    ...
    @PostMapping("/exist")
    public Result existSwiper(@RequestBody @Validated IdParam param) {
        QueryWrapper<SwiperEntity> wrapper = new QueryWrapper<>();
    ...
    @PostMapping("/insert")
    public Result insertSwiper(@RequestBody @Validated SwiperEntity param) {
        QueryWrapper<SwiperEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getSwiperId());
    ...
    @PostMapping("/delete")
    public Result deleteSwiper(@RequestBody @Validated IdParam param) {
        QueryWrapper<SwiperEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
    ...
    @PostMapping("/update")
    public Result updateSwiper(@RequestBody @Validated SwiperEntity param) {
        QueryWrapper<SwiperEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getSwiperId());
    ...
    @PostMapping("/select")
    public Result selectSwiper(@RequestBody @Validated IdParam param) {
        QueryWrapper<SwiperEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(id.toLowerCase(Locale.ROOT), param.getId());
        if (commonService.count(wrapper) == 0) return Result.failure("ID不存在", param.getId());
    ...
    public Result listPageSwiper(@RequestParam Integer page, @RequestParam Integer pageSize) {
        //分页参数
        Page<SwiperEntity> rowPage = new Page(page, pageSize);
        //queryWrapper组装查询where条件
        LambdaQueryWrapper<SwiperEntity> queryWrapper = new LambdaQueryWrapper<>();
    ...
    public void exportExcelSwiper(HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("轮播图", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
        List<SwiperEntity> list = commonService.list();
        response.setHeader("Content-disposition", "attachment;filename*=" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), SwiperEntity.class).sheet("轮播图").doWrite(list);
    }

    @Operation(summary = "导入数据")
    @PostMapping("/importExcel")
    public Result importExcelSwiper(MultipartFile file) {
        try {
            //获取文件的输入流
            InputStream inputStream = file.getInputStream();
            List<SwiperEntity> list = EasyExcel.read(inputStream) //调用read方法
                    //注册自定义监听器，字段校验可以在监听器内实现
                    //.registerReadListener(new UserListener())
                    .head(SwiperEntity.class) //对应导入的实体类
                    .sheet(0) //导入数据的sheet页编号，0代表第一个sheet页，如果不填，则会导入所有sheet页的数据
                    .headRowNumber(1) //列表头行数，1代表列表头有1行，第二行开始为数据行
                    .doReadSync();//开始读Excel，返回一个List<T>集合，继续后续入库操作
            //模拟导入数据库操作
            for (SwiperEntity entity : list) {
