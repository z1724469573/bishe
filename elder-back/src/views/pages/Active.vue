<template>
  ...
  <el-dialog v-model="appendDialog" :close-on-click-modal="false" title="新增" width="700">
    ...
    >
      ...
      <el-form-item label="开始时间" prop="start" :rules="[{ required: true, message: '请选择开始时间' }]">
        <el-date-picker
            v-model="appendForm.start"
            type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
            placeholder="请选择开始时间"
        />
      </el-form-item>
      <el-form-item label="结束时间" prop="end" :rules="[{ required: true, message: '请选择结束时间' }]">
        <el-date-picker
            v-model="appendForm.end"
            type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
            placeholder="请选择结束时间"
        />
      </el-form-item>
...
import api from "@/api";
import {ActiveEditorParams} from "@/api/active";
...
const searchSubmit = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.active.activeSearch(searchForm.keyword).then((res) => {
        tableData.value = res.data;
        ElMessage({
          message: res.message,
...
const formatter = (row: ActiveEditorParams, column: TableColumnCtx<ActiveEditorParams>) => {
  return row.id
}

const tableSelect = ref<ActiveEditorParams[]>([])
const handleSelect = (val: ActiveEditorParams[]) => {
...
const multipleDelete = () => {
  ElMessageBox.confirm('是否批量删除选中数据?', '警告', {
    ...
    for (let i = 0; i < tableSelect.value.length; i++) {
      ...
      if (element.id != null) {
        api.active.activeDelete(element.id).then((res) => {
          ElMessage({
            message: res.message,
            type: res.success ? 'success' : 'error',
            grouping: true,
...
const handleEdit = (index: number, row: ActiveEditorParams) => {
  //@ts-ignore
...
const handleDelete = (index: number, row: ActiveEditorParams) => {
  ElMessageBox.confirm('是否删除这项数据?', '警告', {
    ...
    if (row.id != null) {
      api.active.activeDelete(row.id).then((res) => {
        ElMessage({
          message: res.message,
          type: res.success ? 'success' : 'error',
          grouping: true,
...
const appendSubmit = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.active.activeAppend(appendForm).then((res) => {
        ElMessage({
          message: res.message,
          type: res.success ? 'success' : 'error',
          grouping: true,
...
const editorSubmit = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      //@ts-ignore
      api.active.activeEditor(editorForm.value).then((res) => {
        ElMessage({
          message: res.message,
          type: res.success ? 'success' : 'error',
          grouping: true,
