  ...
  <div>
    ...
    <el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%" height="76.5vh">
      ...
      <el-table-column label="状态" width="100">
        <template #default="scope">
          <el-tag type="success" v-if="scope.row.reviewState===1">正常</el-tag>
          <el-tag type="danger" v-if="scope.row.reviewState===0">禁用</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogSave" title="添加数据" width="33%">
      <el-form :model="formSave">
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="formSave.reviewState">
    ...
    <el-dialog v-model="dialogEdit" title="编辑数据" width="33%">
      <el-form :model="formEdit">
        <div v-for="(item,index) in reviewList" :class="index===0||index===reviewList.length-1?'none':''">
          <el-form-item :label="item.zh">
            <el-input v-model="formEdit[item.en]" placeholder="请输入..." clearable/>
          </el-form-item>
        </div>
        <el-form-item label="状态">
          <el-radio-group v-model="formEdit.reviewState">
            <el-radio :label="1">正常</el-radio>
            <el-radio :label="0">禁用</el-radio>
          </el-radio-group>
        </el-form-item>
...
import {DeleteReviewParams, InsertReviewParams} from "@/api/review";
import {UploadFilled} from '@element-plus/icons-vue'

const reviewList = [
  {zh: '主键', width: 0, en: 'reviewId'},
  {zh: '用户姓名', width: 0, en: 'reviewName'},
  {zh: '日期', width: 0, en: 'reviewDate'},
  {zh: '内容', width: 0, en: 'reviewContent'},
  {zh: '点赞', width: 0, en: 'reviewStar'},
  {zh: '状态', width: 0, en: 'reviewState'},
]
let list = [];
reviewList.filter((e, i) => {
  if (e.en !== 'reviewState') list.push(e);
})
...
//获取分页数据
let tableData: InsertReviewParams[] = [];
const total = ref(0);
const page = ref(1);
const pageSize = ref(13);

function getListPage(page, pageSize) {
  api.review.listPageReview(page, pageSize).then((rs: any) => {
...
//多选
const multipleSelection = ref<InsertReviewParams[]>()
let isSelection = ref(false);
const handleSelectionChange = (val: InsertReviewParams[]) => {
  isSelection.value = true;
  multipleSelection.value = val
}
//打开编辑对话框
const dialogEdit = ref(false)
let formEdit: InsertReviewParams = reactive({})
const handleEdit = (index: number, row: InsertReviewParams) => {
  dialogEdit.value = true;
  formEdit = row;
}
//单个删除
const handleDelete = (index: number, row: InsertReviewParams) => {
  const rowValue: DeleteReviewParams = {id: 0};
  rowValue.id = JSON.parse(JSON.stringify(row)).reviewId;
  api.review.deleteReview(rowValue).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
...
//搜索
const searchValue = ref({}<DeleteReviewParams>);

function search() {
  api.review.selectReview(searchValue.value).then((rs: any) => {
...
let formSave: InsertReviewParams = reactive({})
const empty: InsertReviewParams = reactive({})

function save() {
  const form = JSON.parse(JSON.stringify(formSave));
  api.review.insertReview(form).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
//编辑
function update() {
  api.review.updateReview(formEdit).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
function remove() {
  ...
  for (let i = 0; i < selection.length; i++) {
    api.review.deleteReview({id: selection[i].reviewId}).then((rs: any) => {
      ElMessage({
        message: rs.message,
        grouping: true,
        type: rs.success ? 'success' : 'error',
...
function impact() {
  fileList.value.forEach((file: UploadProps["fileList"]) => {
    ...
    api.review.importExcelReview(formData).then((rs: any) => {
      ElMessage({
...
//导出
function explore() {
  api.review.exportExcelReview().then((rs: any) => {
    const link = document.createElement('a')
    const fileName = "评论表.xlsx";
