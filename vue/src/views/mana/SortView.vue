  ...
  <div>
    ...
    <el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%" height="76.5vh">
      ...
      <el-table-column label="状态" width="100">
        <template #default="scope">
          <el-tag type="success" v-if="scope.row.sortState===1">正常</el-tag>
          <el-tag type="danger" v-if="scope.row.sortState===0">禁用</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogSave" title="添加数据" width="33%">
      <el-form :model="formSave">
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="formSave.sortState">
    ...
    <el-dialog v-model="dialogEdit" title="编辑数据" width="33%">
      <el-form :model="formEdit">
        <div v-for="(item,index) in sortList" :class="index===0||index===sortList.length-1?'none':''">
          <el-form-item :label="item.zh">
            <el-input v-model="formEdit[item.en]" placeholder="请输入..." clearable/>
          </el-form-item>
        </div>
        <el-form-item label="状态">
          <el-radio-group v-model="formEdit.sortState">
            <el-radio :label="1">正常</el-radio>
            <el-radio :label="0">禁用</el-radio>
          </el-radio-group>
        </el-form-item>
...
import {DeleteSortParams, InsertSortParams} from "@/api/sort";
import {UploadFilled} from '@element-plus/icons-vue'

const sortList = [
  {zh: '主键', width: 100, en: 'sortId'},
  {zh: '标题', width: 0, en: 'sortLabel'},
  {zh: '状态', width: 0, en: 'sortState'},
]
let list = [];
sortList.filter((e, i) => {
  if (e.en !== 'sortState') list.push(e);
})
...
//获取分页数据
let tableData: InsertSortParams[] = [];
const total = ref(0);
const page = ref(1);
const pageSize = ref(13);

function getListPage(page, pageSize) {
  api.sort.listPageSort(page, pageSize).then((rs: any) => {
...
//多选
const multipleSelection = ref<InsertSortParams[]>()
let isSelection = ref(false);
const handleSelectionChange = (val: InsertSortParams[]) => {
  isSelection.value = true;
  multipleSelection.value = val
}
//打开编辑对话框
const dialogEdit = ref(false)
let formEdit: InsertSortParams = reactive({})
const handleEdit = (index: number, row: InsertSortParams) => {
  dialogEdit.value = true;
  formEdit = row;
}
//单个删除
const handleDelete = (index: number, row: InsertSortParams) => {
  const rowValue: DeleteSortParams = {id: 0};
  rowValue.id = JSON.parse(JSON.stringify(row)).sortId;
  api.sort.deleteSort(rowValue).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
...
//搜索
const searchValue = ref({}<DeleteSortParams>);

function search() {
  api.sort.selectSort(searchValue.value).then((rs: any) => {
...
let formSave: InsertSortParams = reactive({})
const empty: InsertSortParams = reactive({})

function save() {
  const form = JSON.parse(JSON.stringify(formSave));
  api.sort.insertSort(form).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
//编辑
function update() {
  api.sort.updateSort(formEdit).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
function remove() {
  ...
  for (let i = 0; i < selection.length; i++) {
    api.sort.deleteSort({id: selection[i].sortId}).then((rs: any) => {
      ElMessage({
        message: rs.message,
        grouping: true,
        type: rs.success ? 'success' : 'error',
...
function impact() {
  fileList.value.forEach((file: UploadProps["fileList"]) => {
    ...
    api.sort.importExcelSort(formData).then((rs: any) => {
      ElMessage({
...
//导出
function explore() {
  api.sort.exportExcelSort().then((rs: any) => {
    const link = document.createElement('a')
    const fileName = "新闻分类表.xlsx";
