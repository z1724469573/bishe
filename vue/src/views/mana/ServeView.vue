<template>
  <div>
    ...
    <el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%" height="76.5vh">
      ...
      <el-table-column prop="serveCover" label="封面" align="center">
        <template #default="scope">
          <el-popover effect="light" trigger="hover" placement="top" width="auto">
            <template #default>
              <div>{{ scope.row.serveCover }}</div>
            </template>
            <template #reference>
              <el-avatar shape="square" :size="50" :src="scope.row.serveCover"/>
            </template>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="100">
        <template #default="scope">
          <el-tag type="success" v-if="scope.row.serveState===1">正常</el-tag>
          <el-tag type="danger" v-if="scope.row.serveState===0">禁用</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogEdit" title="编辑数据" width="33%">
      <el-form :model="formEdit">
        <div v-for="(item,index) in serveList" :class="index===0||index===serveList.length-1?'none':''">
          <el-form-item :label="item.zh">
            <el-input v-model="formEdit[item.en]" placeholder="请输入..." clearable/>
          </el-form-item>
        </div>
        <el-form-item label="状态">
          <el-radio-group v-model="formEdit.serveState">
            <el-radio :label="1">正常</el-radio>
            <el-radio :label="0">禁用</el-radio>
          </el-radio-group>
        </el-form-item>
...
import {DeleteServeParams, InsertServeParams} from "@/api/serve";
import {UploadFilled} from '@element-plus/icons-vue'

const serveList = [
  {zh: '主键', width: 60, en: 'serveId'},
  {zh: '封面', width: 30, en: 'serveCover'},
  {zh: '标题', width: 150, en: 'serveLabel'},
  {zh: '内容', width: 500, en: 'serveContent'},
  {zh: '状态', width: 20, en: 'serveState'},
]

let list = [];
serveList.filter((e, i) => {
  if (e.en !== 'serveCover'&&e.en !== 'serveState') list.push(e);
})
...
//获取分页数据
let tableData: InsertServeParams[] = [];
const total = ref(0);
const page = ref(1);
const pageSize = ref(13);

function getListPage(page, pageSize) {
  api.serve.listPageServe(page, pageSize).then((rs: any) => {
...
//多选
const multipleSelection = ref<InsertServeParams[]>()
let isSelection = ref(false);
const handleSelectionChange = (val: InsertServeParams[]) => {
  isSelection.value = true;
  multipleSelection.value = val
}
//打开编辑对话框
const dialogEdit = ref(false)
let formEdit: InsertServeParams = reactive({})
const handleEdit = (index: number, row: InsertServeParams) => {
  dialogEdit.value = true;
  formEdit = row;
}
//单个删除
const handleDelete = (index: number, row: InsertServeParams) => {
  const rowValue: DeleteServeParams = {id: 0};
  rowValue.id = JSON.parse(JSON.stringify(row)).serveId;
  api.serve.deleteServe(rowValue).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
...
//搜索
const searchValue = ref({}<DeleteServeParams>);

function search() {
  api.serve.selectServe(searchValue.value).then((rs: any) => {
...
let formSave: InsertServeParams = reactive({})
const empty: InsertServeParams = reactive({})

function save() {
  const form = JSON.parse(JSON.stringify(formSave));
  api.serve.insertServe(form).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
//编辑
function update() {
  api.serve.updateServe(formEdit).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
function remove() {
  ...
  for (let i = 0; i < selection.length; i++) {
    api.serve.deleteServe({id: selection[i].serveId}).then((rs: any) => {
      ElMessage({
        message: rs.message,
        grouping: true,
        type: rs.success ? 'success' : 'error',
...
function impact() {
  fileList.value.forEach((file: UploadProps["fileList"]) => {
    ...
    api.serve.importExcelServe(formData).then((rs: any) => {
      ElMessage({
...
//导出
function explore() {
  api.serve.exportExcelServe().then((rs: any) => {
    const link = document.createElement('a')
    const fileName = "服务表.xlsx";
