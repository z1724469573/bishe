<template>
  <div>
    ...
    <el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%" height="76.5vh">
      ...
      <el-table-column prop="activeCover" label="封面" align="center">
        <template #default="scope">
          <el-popover effect="light" trigger="hover" placement="top" width="auto">
            <template #default>
              <div>{{ scope.row.activeCover }}</div>
            </template>
            <template #reference>
              <el-avatar shape="square" :size="50" :src="scope.row.activeCover"/>
            </template>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="100">
        <template #default="scope">
          <el-tag type="success" v-if="scope.row.activeState===1">正常</el-tag>
          <el-tag type="danger" v-if="scope.row.activeState===0">禁用</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogSave" title="添加数据" width="33%">
      <el-form :model="formSave">
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="formSave.activeState">
    ...
    <el-dialog v-model="dialogEdit" title="编辑数据" width="33%">
      <el-form :model="formEdit">
        <div v-for="(item,index) in activeList" :class="index===0||index===activeList.length-1?'none':''">
          <el-form-item :label="item.zh">
            <el-input v-model="formEdit[item.en]" placeholder="请输入..." clearable/>
          </el-form-item>
        </div>
        <el-form-item label="状态">
          <el-radio-group v-model="formEdit.activeState">
            <el-radio :label="1">正常</el-radio>
            <el-radio :label="0">禁用</el-radio>
          </el-radio-group>
        </el-form-item>
...
import {DeleteActiveParams, InsertActiveParams} from "@/api/active";
import {UploadFilled} from '@element-plus/icons-vue'

const activeList = [
  {zh: '主键', width: 100, en: 'activeId'},
  {zh: '标题', width: 0, en: 'activeLabel'},
  {zh: '开始', width: 0, en: 'activeStart'},
  {zh: '结束', width: 0, en: 'activeEnd'},
  {zh: '地址', width: 0, en: 'activeAddress'},
  {zh: '电话', width: 0, en: 'activePhone'},
  {zh: '内容', width: 0, en: 'activeContent'},
  {zh: '封面', width: 0, en: 'activeCover'},
  {zh: '状态', width: 0, en: 'activeState'},
]
let list = [];
activeList.filter((e, i) => {
  if (e.en !== 'activeCover'&&e.en !== 'activeState') list.push(e);
})
...
//获取分页数据
let tableData: InsertActiveParams[] = [];
const total = ref(0);
const page = ref(1);
const pageSize = ref(13);

function getListPage(page, pageSize) {
  api.active.listPageActive(page, pageSize).then((rs: any) => {
...
//多选
const multipleSelection = ref<InsertActiveParams[]>()
let isSelection = ref(false);
const handleSelectionChange = (val: InsertActiveParams[]) => {
  isSelection.value = true;
  multipleSelection.value = val
}
//打开编辑对话框
const dialogEdit = ref(false)
let formEdit: InsertActiveParams = reactive({})
const handleEdit = (index: number, row: InsertActiveParams) => {
  dialogEdit.value = true;
  formEdit = row;
}
//单个删除
const handleDelete = (index: number, row: InsertActiveParams) => {
  const rowValue: DeleteActiveParams = {id: 0};
  rowValue.id = JSON.parse(JSON.stringify(row)).activeId;
  api.active.deleteActive(rowValue).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
...
//搜索
const searchValue = ref({}<DeleteActiveParams>);

function search() {
  api.active.selectActive(searchValue.value).then((rs: any) => {
...
let formSave: InsertActiveParams = reactive({})
const empty: InsertActiveParams = reactive({})

function save() {
  const form = JSON.parse(JSON.stringify(formSave));
  api.active.insertActive(form).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
//编辑
function update() {
  api.active.updateActive(formEdit).then((rs: any) => {
    ElMessage({
      message: rs.message,
      grouping: true,
      type: rs.success ? 'success' : 'error',
...
function remove() {
  ...
  for (let i = 0; i < selection.length; i++) {
    api.active.deleteActive({id: selection[i].activeId}).then((rs: any) => {
      ElMessage({
        message: rs.message,
        grouping: true,
        type: rs.success ? 'success' : 'error',
...
function impact() {
  fileList.value.forEach((file: UploadProps["fileList"]) => {
    ...
    api.active.importExcelActive(formData).then((rs: any) => {
      ElMessage({
...
//导出
function explore() {
  api.active.exportExcelActive().then((rs: any) => {
    const link = document.createElement('a')
    const fileName = "活动表.xlsx";
